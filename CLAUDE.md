...
- **客户面是从标题里认出来的，认的时候不许把一串数字从中间切开**。客户名大量是
  ...
  里的数字），用 [scripts/repair_issue_customers.py](backend/scripts/repair_issue_customers.py)
  重算历史（默认只读预演，`--apply` 才写）。它**不碰 `issue_snapshot_flows`**：
  差分按缺陷编号算，与客户面无关，重算反而会造出一批假新增/假解决。
