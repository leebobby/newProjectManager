...
它从**真实 DOM 里取侧栏菜单项**（跑之前先把二级菜单全展开——不展开的话
「专项管理」「客户面状态」底下的页面既点不到、也就不在覆盖里），
新加的页面自动被覆盖，不必回来改测试；
...
| 档位 | 依赖 | 适用 |
| --- | --- | --- |
| **登录用户**（协作编辑域） | `Depends(get_current_user)` | 日常填报与记录：进展、事务、风险、问题条目、出差、调试版本、领域内容、领域遗留问题、关键特性、专项总览点灯…… |
...
`/specials/overview`（侧栏「专项管理 · 总览」，登录用户可见）是一张跨专项的横表：
序号 / 关键专题 / 目标 / 风险（点灯）/ 关键进展 / 关键风险和措施 / 责任人。

**「自动提取」是这张表存在的理由**——目标＝`content.goal`，关键进展＝
`content.progress_summary`，关键风险和措施＝`special_risks` 里 `status=open` 的行
（`content`＝风险、`progress`＝措施，**两列分开给前端**，合成一段字符串就再也拆不开了），
责任人＝`specials.owner`。**不要给总览加录入入口**：哪一列一旦能在这儿单独填，
它就会和详情页对不上，而两边看着都对。唯一的例外是点灯（下面那条）。

- 文本列**服务端剥 HTML、但不截断**：这张表是横着扫的，标签留着每行高矮不一；
  截断则是把内容弄丢了，改成前端 CSS 三行夹断 + 原生 `title` + 「展开全部文字」开关。
  风险一格最多铺 3 条（超期的排前面），多出来的写明「另 N 条」并可点进专项——
  同 `pptx_utils` 的 `clip_cols`：截断可以，但要截对头、要说清楚少了几条。
- `GET /overview` **必须注册在 `GET /{sid}` 之前**。反了的话 `"overview"` 会被当成
...
| `gray`（未评估） | 一条风险行都没登记 |
| `green` | 登记过，但全部已闭环 |
| `yellow` | 有未闭环的，且都还没过计划闭环时间 |
| `red` | 有未闭环的，且至少一条**已过**计划闭环时间 |

- **「一条都没登记」不算绿**。那是"还没人评过"，不是"评过、没风险"；记成绿的话，
  最该被追着去补风险的那几个专项在总览上看着比谁都干净（同领域管理的
  「超期未知」vs「无超期」：算不出来要如实说算不出来）。
- 「**超过**」才算，当天到期不算——记成超期会让人白紧张一天。日期解析直接用
  `_issue_source.parse_plan_date()`，**不要在这儿另写一份**：两处分叉的表现是
  同一个日期在两个页面上一个算超期一个不算。
- **没填计划闭环时间的未闭环行只顶到黄、不顶红**：那是「没排期」，不是「排了没做到」。
  混成一档会让一批从来不填日期的专项长期挂红，而红灯一多就没人看了，真红的那个反而淹掉。
- **刻意不掺里程碑延期**。延期的里程碑确实是风险信号，但掺进来这盏灯就再也说不清出处
  ——页面写着红、风险表里一条超期都没有，没人查得动。要让里程碑影响它，
  正确做法是去风险表里登一行。

**人工覆盖**存在 `special_contents.overview_light`（**存 key 不存中文**，空＝自动，
校验走 `enums.norm_special_light()`——它不走 `_norm_choice`，因为空是合法取值而不是
"不修改"或"落默认值"，同 `norm_domain_risk_level()`）：

- 写权限＝**登录用户**（协作编辑域的日常填报）。要 admin 代拨的话，灯会一直停在上上周。
- 并发共用 `content.version` 这一个乐观锁（本来就是同一行），他人持编辑锁时返回 **423**。
- 接口把 `light` / `light_auto` / `light_manual` / `light_reason` **四个都返回**。
  只给最终值的话，风险都闭环了灯还是红，没人分得清是覆盖忘了清还是规则算错了。
- 前端 `SpecialOverview.vue` 里的 `LIGHT_LABELS` 与后端
  `enums.SPECIAL_OVERVIEW_LIGHT_LABELS` **两端各一份，必须同步**；漏一档的表现是
  那一档的行灯位显示成空白。**这套灯与另外两套点灯是三回事，别互相合并**：
  自由表格的点灯列（`GRID_LIGHT_COLORS` / `gridLight.js`）是用户配的单元格格式，
  导出里的状态点灯（`brand.STATUS_FILLS`）认的是进展状态词。
...
- 用 `date`/`datetime.now()` 生成的**日期字符串**（`snapshot_date`、导出文件名、报告日期）
  本来就是本地时间，不要动。
- **自由格式日期串**（DTS 的「预计闭环时间」、里程碑日期这类人/外系统填进来的列）
  统一用 `timeutil.parse_plan_date()` 解析，`routers._issue_source` 原样再导出同一个
  函数（既有调用点与文档引用不变）。实现放在 `timeutil` 是因为它不依赖
  models/database——`special_layout` / `xlsx_utils` 这些不该拖起 ORM 的模块也要用它。
  **认不出来的一律算「没填」而不是算「没超期」**，别在别处另写一份。
...
- **专项图文混排导出**用 [xlsx_utils.py](backend/xlsx_utils.py)。周报的层次是
  ...
  而它表达的其实只是某一列填了「已完成」。版面回归见
  [tests/test_special_xlsx_layout.py](backend/tests/test_special_xlsx_layout.py)。
  四条硬规则：
  - **一个专项导出成一张表**，所有分段（含自定义自由表格）按页面顺序往下排。
    自定义表格原先因为列宽对不上被丢到独立工作表、主表只留一行「→ 见工作表「X」」：
    一份周报打开是四五个页签，顺序、章节号与上下文全断在那行指引上。
  - 全表是 **36 列等宽窄网格**（`_NCOL` / `_BASE_W`），每张表按**列宽比例**
    （`_spans_from_ratios`，同 `pptx_utils` 的 `col_ratios`）合并上去。
    列宽一样宽是"能并存"的前提——早先 6 个宽窄不一的列（`[6,36,30,12,14,10]`）
    只喂得饱事务表，5~8 列的自由表格塞进去必然变形，这才有了独立工作表那套。
    别改回按字符数写死列宽：加一列、改个标题就再也对不上。
  - **合并区的样式要逐格写**（`band()` / `narrative()` / `_frame()` 都是这么写的）：
    xlsx 文件里合并区**只留左上角那一格**，而 Excel 画合并区的边框是**按边上每一格**
    取的——只写左上角，那道线就只在 A 列那么宽处露出一小截（现场反馈是"每个段落
    后面一个小短线"），右边则整个没有框。底色是例外：合并区照左上角那一格显示。
    **别拿 openpyxl 读回来的对象验证这件事**：它的 reader 会自己把左上角的边框铺到
    合并区四周，读回来看着是完整的框，而文件里根本没有。要验就解开 xlsx 数 `<c>`
    标签，见 `tests/test_special_xlsx_layout.py` 的 `_raw_borders()`。
  - **外框按「分段」画**（标题行 + 内容一个框），不是每张表各画一个：
    章节标题、正文、表格三层里只有表格自带细网格，没有外框时整页是一堆浮着的横条。
  - **图片是浮在格子上的、不占行**，必须按 `_rows_for_px()` 预留（默认行高 15 磅
    ＝ **20px**）。少留下一段标题被图压住，多留则图后面空出半屏白，
    看着像"这一段没导出来"。
- **里程碑画成时间轴图片**（`_render_milestone_image`，PIL）。都是
  "文件能打开、只是读不出来"的那类：
  - **日期先归一**（`special_layout.milestone_date_text()`，周报与 Excel 共用一份）。
    这一列在库里是自由字符串，`2026-05-20T00:00:00` / `2026/7/1` / 毫秒时间戳都真实
    存在过，原样画上去一个串进隔壁格子、一个和别人对不齐，而每个单独看都"没错"。
    认不出来的**原样留着**（多半是「6月中旬」这种人写的话），别丢。
  - **折行不拆西文单词**（`_wrap_by_width`）：逐字符折行把 "Release" 断成
    「Releas / e」，中文看不出问题，英文一眼就是坏的。只有一个词本身装不下才劈开。
  - **槽宽按最长的那一格算、超过表宽就换行摆**，不是写死间距、也不是把图一路加宽：
    Excel 里图片不跟着列走，画到 3000px 宽就是横着甩出表外面老远。
  - 找不到能渲染中文的字体时**整体退回表格形式**（`_load_pil_font` 返回 None），
    绝不回退 PIL 默认字体——那不含中文，画出来是一排方块。
