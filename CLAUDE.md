...
责任人＝`specials.owner`。**不要给总览开自己的存储**：哪一列一旦在这儿单独存一份，
它就会和详情页对不上，而两边看着都对。

要拦的是「单独存一份」，不是「不许在这儿改」。「关键进展」与「关键风险和措施」
在总览上是**可以就地编辑的**，但走的是详情页那两个接口（`PUT /{sid}/content` 的
`progress_summary`、`PUT /risks/{item_id}`），改的是详情页那个字段本身，
所以两边永远是同一个值，周报与导出也跟着变。两条配套规则：

- **编辑前要回源拿原始富文本**（`GET /specials/{sid}`）。总览接口给的是
  `_strip_html()` 剥过标签的文本，拿它存回去等于把别人排的版一次抹平，
  而保存时一点提示都没有。
- **风险行的编辑表单只有一份**：[SpecialItemDialog.vue](frontend/src/components/SpecialItemDialog.vue)，
  详情页与总览共用。两处各写一份的表现是加一个字段只加在其中一页上，
  另一页一保存就把它抹掉了（`exclude_unset` 那侧把没传当成不修改）。

新增列时照这个办：要么从已有字段推，要么写回已有字段。点灯是唯一「只属于总览」的
那一格（下面那条）——它答的是"这周这个专项该是什么颜色"，详情页里没有对应字段。
...
| 时间轴 `VersionTimeline`：泳道 / 节点 | 大版本 / **版本**（可切到迭代版本，见下） |
...
- **时间轴的节点粒度可切三档**（只看大版本 / 版本 / 迭代版本 B 版），但
  **泳道的起止与粒度无关**：切粒度时线不能跟着挪，否则同一个大版本在两种视图里
  起点不一样，看着像数据变了。粒度只决定线上画哪一层的点。
- **「未发布」靠换色，不靠空心**：计划中还没发的节点画成实心琥珀 `#E6A23C`
  （`PLANNED`），空心圈在缩小和打印时几乎看不见，而"这个版本还没发"恰恰是看这张图的人
  最想一眼扫到的事。配套地**琥珀已经从泳道配色 `PALETTE` 里摘掉了**——不摘的话，
  落在琥珀色泳道上的已发布节点会和未发布节点同色，而那一条泳道单独看完全正常。
  图例画进 `<svg>` 里而不是写在页面上，导出的那张图才带得走这条说明。
- **横轴窗口有两条路，别合并**（`explicitWindow()` 返回 null 与否）：
  **选了时间范围**（近 N 月 / 按月份区间）＝原样画出来，哪怕大半是空的——那是查看完整
  声明范围的唯一入路；**「全部」＝只画有内容的时间段**，收窄到「第一个节点前 14 天 ~
  最后一个节点 + `TAIL_DAYS`」。一个大版本声明了三年、版本却全挤在最后半年时，
  从声明起点铺满会把大半张图让给空白，几个版本挤成一团谁也读不出来。
  合成一条的表现是"我明明选了 2024 年 1 月，图却从 2026 年 3 月开始"。
  收窄**只是画法**：`range_start` 没有被改写，起点早于窗口的泳道左端画一个 `«`
  （带 tooltip），表示"开始时间在窗口之外"而不是被截断；那条"这之前还没开始"的引入虚线
  此时**不画**——它的意思和 `«` 正相反。被窗口挡在外面的节点数要如实报
  （`nodeClipped`），只筛不报的表现是"这个版本怎么不在图上"。
- **图上点少时整体放大一档**（`SPARSE_NODE_MAX`，默认 12）：泳道高 64→88、节点半径 +2、
  版本号字号 9→11。**时间比例一个字都不动**，放大的只是画笔——稀疏时不放大的话，
  整张图就是一根细线上孤零零几个点，版本号小到得凑过去看。节点标签的上下偏移与引线
  长度都从 `nodeR` / `labelFS` 推，写死的话放大之后标签会压在点上。
- 月份刻度按窗口宽度自动隔 1 / 3 / 6 个月标一次：每月一格在宽窗口下会糊成一片。
- **泳道末端比最后一个节点多伸 5 天并画成箭头**（`TAIL_DAYS`）：圆点是"到此为止"，
  而版本线的末端是"还在往前走"；最后一个版本正好压在端点上时看着像被截断。
  这**只是画法不是数据**——`range_end` 没有被改写，箭头的悬停提示里写的仍是真日期。
...
## WBS：挂专项 / 机台调试，层数不限，父行只汇总

一份 WBS（`wbs_plans`）挂在**一个专项**或**一台机台的调试**上，**不挂版本**：
一个版本里同时跑着好几件事（几个特性调试、几台机台的现场调试），挂版本就得把它们
混在一棵树里，「这段是谁的活」从数据上分不出来。与里程碑的分工是——里程碑
（`roadmap_phases` / `roadmap_milestones`）是项目级的粗粒度计划，WBS 把某一件事
往下拆到能派活的粒度。实现在 [routers/wbs.py](backend/routers/wbs.py)，改之前先读它顶部说明。

树（`wbs_items`）**同一张表靠 `parent_id` 分层，层数不限**。不做成「分组表 + 工作包表」
两张：真实的拆解深度是拆到才知道的，`2.1` 今天是工作包，明天拆成 `2.1.1` 就成了分组，
两张表的话这一步要跨表搬行。

- **能不能填，看它有没有子行，不看它在第几层。** 叶子行填人天 / 完成度 / 状态 / 计划起止；
  只要挂了子行，这几项一律由 `_rollup()` 从叶子汇总，**不入库、写入也被丢掉**
  （`update_item` 里静默 pop 而不是报 400——前端本就把这几格禁掉了，能走到这儿的多半是
  并发下别人刚加了子行，报错只会让人一脸茫然地丢掉整次保存）。父行能单独填的话，
  把 `2.1` 拆开之后父子两个数就对不上了，而两边看着都对。
- **完成度按人天加权**，分子分母都只数**叶子**：`Σ(人天×完成度) ÷ Σ人天`。中间层不重复
  计入，所以「2」的数字和「2.1.1 + 2.1.2 + 2.2 + …」永远对得上。按条数算的话，
  一个 20 人天的包和一个 1 人天的包各算一条。
- **计划起止也是汇总**：父行取子行的最早开始与最晚完成。父行能单独填的话，
  「父行写着 9 月底完成、底下有个子任务排到 10 月」这种事没人拦得住。
- **「已变更 / 不涉及」整行不进统计**（`enums.WBS_UNCOUNTED_STATUSES`），**但排除了几条、
  多少人天要如实报出来**（`excluded` / `excluded_days`）。只筛不报的表现是「数字怎么小了
  一截」，而没人说得清少的是哪些（同 `unassigned` / `overdue_unknown` / `match_rate`）。
  行本身留着——那是计划的一部分，也是别人回头看「为什么没做」的唯一线索。
- **编号（`code`，1.2.3）不入库**，出接口时按 `parent_id + sort_order` 现算。存下来的话，
  上移一行、加一个子项之后编号就和位置对不上了，而每一行单独看都合法。
- **待补录只判叶子**（`_issues()`）：父行那几项本来就是汇总来的，判它等于骂错人。
  「已过计划完成日」**只提示、不自动改状态**：延期是人来认的，系统替他认了的话，
  页面上写着「已延期」而当事人根本不知道是谁标的。「超过」才算，当天到期不算。
- **改层级走 `/items/{id}/move`，不做成 `WbsItemUpdate` 里的普通字段**：那一步还要重排
  `sort_order` 并**防环**。把一行挂到自己的子孙下面，那棵子树就从根上够不着了——
  页面表现是「这几行凭空消失」，而库里一行没少，最难查的那类。
- `/reorder` 整体重写某一层的顺序（同版本三层）：混进别的父级的行返回 **400**，
  静默忽略会让人以为排序时灵时不灵；列表里没提到的兄弟排到后面。
- **套模板只增不删**（`apply-template`，同专项模板的语义）：按名字认领已有的分组，
  重复套幂等，模板外的行与已填的内容一律保留。
- 权限：建 / 改 WBS、树里的所有增删改＝**登录用户**（要 admin 代建的话，现场调试那边就
  没人建了）；**删整份 WBS ＝仅 admin**，按「删掉的是什么」定档——那是别人跟了几个月的计划。
- **写操作一律回整份详情**（树 + 汇总），前端拿它整份覆盖、不自己就地加。两端各算一次
  迟早对不上。回归见 [tests/test_wbs.py](backend/tests/test_wbs.py)。
...
**交付范围 ＝ `by-version` 两个接口**（「版本管理 → 合入需求」那个弹窗）。
它回的是 `{items, changed}` 而不是裸数组：

- **「已变更」的行不进交付范围**。那条需求本轮不做了，留在版本的合入清单里会让评审
  按一个虚高的条数去核（与度量看板的 `_split_changed` 同一份口径，见
  `routers/_req_progress.py`）。
- **剔了几条要如实报出来**（`changed`），页面上写明「另有 N 条标了已变更，不计入本版本的
  交付范围」。只剔不报的表现是"这个版本怎么少了两条"，而没人说得清少的是哪些
  （同 `unassigned` / `overdue_unknown` / `match_rate`）。

**批量改版本 / 批量挪迭代**收口在
[routers/_req_bulk.py](backend/routers/_req_bulk.py)，领域需求与产品需求两张表共用；
前端共用 [RequirementBulkBar.vue](frontend/src/components/iteration/RequirementBulkBar.vue)。
各写一份的表现是「在一个 Tab 挪走会重排序号、在另一个 Tab 不重排」，而两边单独看都正常。

- **批量照样走乐观锁**：每条都要带自己的 `version`。撞上的**单独失败、不整批回滚**
  ——整批失败的话，一条被别人动过就得从头再选一遍。所以是「能改的改掉，改不动的
  逐条报出来」，前端把 `conflicts` 摊开给人看：只弹一句"保存成功"的话，
  人以为 12 条全改了，其实只改了 9 条。
- **挪迭代要重判重**：目标迭代里可能已经手工补录过同一条（上个月没做完、这个月又录了
  一条）。不判的话两条并存，在度量里是实打实的分母，而每一行单独看都合法（同 `_req_dedup`）。
- **挪迭代要把 `seq` 重算成目标迭代的末位**，带着原序号过去会插进目标那一堆的中间，
  看着像随机落点（同版本三层改挂父级的 `_tail_sort_order()`）。
- **12 月 +1 要跨年**（`_shift()` 走「年×12+月」的绝对月序），就地取模会算出 13 月。
- **改版本与挪迭代互斥**，一次只干一件事：混在一起时一条需求既换了版本又换了迭代，
  出了问题没人说得清是哪一步干的。
- 版本在页面上按**两级**选：先大版本、再它底下的构建。行上存的是构建号
  （`C10SPC101B003`），而人脑子里记的是大版本——把上百个构建铺在一个下拉里，
  挑对的那个全靠肉眼扫前缀。
- 回归见 [tests/test_req_bulk.py](backend/tests/test_req_bulk.py)。

## 产品需求 ↔ 领域需求：拆解关联是**独立一张表**

一条产品需求拆到几个领域去做，关系落在 `iteration_requirement_links`
（[routers/iteration_req_links.py](backend/routers/iteration_req_links.py)），
**不是在领域需求上加一个「父产品需求」外键**。三个理由，少一个都会在别处冒出来：

- **一条领域需求可以承接多条产品需求**（平台/公共模块的改动尤其常见）。单外键的话，
  第二条产品需求的拆解看着就是"还没拆"，而没人说得清为什么明明有人在做。
- **挂接不该动需求行本身**。两张需求表都带 `version` 乐观锁，在产品需求页挂一条领域需求
  如果要去 `PUT` 那条领域需求，就会撞上正在编辑那一行的人（409），而挂接跟他填的内容
  根本不冲突。关联表有自己的行，挂 / 解挂都不碰任何一条需求。
- 解挂＝删一行关联，不是把别人行上的字段清空。

配套硬规则：

- **关联表里不存任何汇总数字**。「这条产品需求拆下去做到哪儿了」由领域需求行现算，
  领域需求那边一改，产品需求页上的数字就跟着变。存一份的话它会停在上一次挂接的时刻，
  而两边看着都对（同专项总览「不要给总览开自己的存储」）。
- **进展口径与度量看板共用一份**：[routers/_req_progress.py](backend/routers/_req_progress.py)
  （进展字段清单 / 权重表 / `is_done` / `split_changed`）。`metrics.py` 按私有别名再导出，
  既有调用点不变。两处各写一份的表现是同一条需求在看板里算完成、在拆解汇总里算没完成。
- **产品需求自己那 7 个进展子项不被拆解汇总覆盖，也不该被覆盖**。它答的是「这条产品需求
  的串讲 / 澄清 / 测试结论走到哪一步」，拆解汇总答的是「底下几条领域需求做完没有」——
  两个问题。页面上是并排两列，不是一列。
- **跨迭代关联允许，但要标出来**（`cross_iteration`）。同一条需求本轮没做完、下个月接着排
  是正常的（同 `_req_dedup` 的跨迭代口径）；限定同迭代会让产品需求在下个月凭空变成"未拆解"。
  候选检索默认只在当前迭代里找，要挂别的迭代得显式勾「含其它迭代」——免得手一滑挂到一条
  同名的老需求上。
- **`version_mismatch` 只断言「不一致」，从不断言「一致」**：两边都推得出计划交付**版本**
  （release 层，FK 优先、字符串回退，同 `_req_scope`）且不是同一个才标。推不出来的一律不标，
  也不显示「一致」——算不出来要如实说算不出来（同 `overdue_unknown` / `match_rate`）。
  刻意不判「谁更晚」：那要跨版本排序，推错了页面上写着"延期"而其实没有。
- **未拆解的条数要如实报出来**（产品需求 Tab 工具栏那条黄字）。不报的话，一条领域需求都没挂
  的产品需求和"拆完了"长得一样，而它们正是最该被追着去拆的那批。
- **两个 Tab 吃同一个列表接口**（`GET /api/iteration-req-links?iteration_id=`，返回两侧任一侧
  在该迭代的全部关联），各自按 `product_req_id` / `domain_req_id` 切片。各拉各方向的话，
  同一条跨迭代关联会在一个 Tab 里看得见、另一个看不见，而两边看着都对。
- **挂接表单只有一份**：[RequirementLinkDialog.vue](frontend/src/components/iteration/RequirementLinkDialog.vue)，
  按 `side` 决定当前这条在哪一侧，表格里始终显示对面那一条。两处各写一份的表现是加一个字段
  只加在其中一页上（同 `SpecialItemDialog.vue`）。
- 权限：读 / 增 / 改 / 删都是**登录用户**。删除按「删掉的是什么」定档——解挂丢掉的是一条挂接
  关系，两条需求一行不动，重新挂上就是再点一次。
- 回归见 [tests/test_req_links.py](backend/tests/test_req_links.py)。
...
DI ＝ 按严重程度加权求和,**致命 10 / 严重 3 / 一般 1 / 提示 0.1**。权重表
`SEVERITY_WEIGHTS` 与 `row_score` / `weighted_score` / `score_by` 全部收口在
[routers/_issue_source.py](backend/routers/_issue_source.py)——领域总览、度量看板、
问题单管理三处共用。**前端也不另存一份**:权重随 `snapshot-detail` 的
`di_weights` 发下去,页面照着算。各写一份的表现是同一批单在三个页面上三个 DI,
而每一个单独看都挺合理。

- **认不出的级别记 0 分,但那一条仍然计入条数**。丢掉的话条数和 DI 对不上,
  两个数单独看都正常;记 0 则表现为「条数涨了 DI 没涨」,一眼看得出是词表没对上。
- **`score_by()` 的分组口径必须和 `issues._count_by()` 一模一样**:取值为空的行
  两边都**整条跳过**,不落「未标注」那种兜底桶。分法差一点,同一张表里条数按一种
  分法、DI 按另一种,两列加起来都还对得上,只有分到哪一行不一样——没人会去核。
- **合计 DI 一律按「严重程度」那一维加总**,与页面选的维度无关:级别为空的行会被
  跳过,但那种行的 DI 本来就是 0(不在权重表里),所以各档之和 ＝ 整份快照的 DI,
  一分不差。换成按客户面加总就不成立——标题匹配不到客户的单整条不进那一维。
- DI **和条数一起落在 `issue_snapshot_stats.score`**,因为**趋势只读库、不碰明细文件**;
  看图时现算就得把每天的明细 JSON 全读一遍,而那正是当初把数字入库要避开的事。
- **`score` 可空,且空 ≠ 0 分**:这一列是后加的(迁移 `0017`),之前采的快照没算过。
  接口对这些天回 `null` 并列进 `di_missing_dates`,页面明说「还没回算」,图上断线。
  记 0 的话那一段贴着零轴,看着像那几天确实没缺陷(同 `overdue_unknown` /
  `match_rate`:算不出来要如实说算不出来)。回算走
  [scripts/backfill_issue_di.py](backend/scripts/backfill_issue_di.py)(默认只读预演)。
- **重建 `issue_snapshot_stats` 的地方都要连 DI 一起写**——`_take_snapshot()` 与
  `scripts/repair_issue_dimensions.py` 都是删掉再重建。只写 `count` 的话,重算脚本
  跑一次就把该维度的 DI 抹回空,而条数一切正常,看着完全不像是它干的。
- 空快照(`total=0`)没有任何维度行,那**不是「没算过」**,它的 DI 就是 0。
  不特判的话每个空快照都会被报成待回算,页面上顶着一条永远消不掉的黄条。
- 趋势接口**一次返回条数与 DI 两套数**,前端切指标不重新请求(同「切标签不重新请求」)。
- 回归见 [tests/test_issue_di.py](backend/tests/test_issue_di.py)。
...
`stakeholder_feature_owners` 一行＝一个**关键特性在一个项目上**的 FO / SE / TFO
（[routers/stakeholders.py](backend/routers/stakeholders.py) 末段）。同一个特性在不同项目上
责任人常常不是同一批人，所以项目是**行上的一个维度**，不是给每个项目各开一份名单。

- **FO / SE 留空＝继承 `key_features` 上那个特性级的责任人**，出接口时把
  `fo_effective` / `fo_inherited` 分开给，页面照着标「继承」。只给最终值的话，
  改的人分不清这个名字是本项目填的还是从特性上来的，会以为自己在改本项目的值
  （同 `domain_issue_targets` 的通用兜底目标）。**前端不再算一遍继承**——
  两处各写一份的表现是同一格在页面和接口里不是同一个人，而两边看着都对。
- 编辑时回填的是**这一行自己填的值**（`fo`）而不是 `fo_effective`：把继承来的名字
  灌进输入框，一保存就抄成了本行的值，从此再也不跟着特性走了。
- **TFO 在特性表里没有对应列，空就是空，不继承**。
- `project_id` 为空＝通用/未指定项目的一行，**不参与继承**：两级兜底会让
  「这个格子里的名字到底从哪儿来」没人说得清。
- 同一个 (特性, 项目) 只能有一行，重复返回 **400**。判重在路由里做而不是靠数据库
  唯一约束：SQLite 里 NULL 互不相等，带空 `project_id` 的行照样能建出好几条，
  约束只挡住一半，表现是"有时候拦得住有时候拦不住"。
- 权限：读＝登录用户；增删改＝**仅 admin**（同这一页的另两张表，主数据一档）。
- 回归见 [tests/test_stakeholder_feature_owners.py](backend/tests/test_stakeholder_feature_owners.py)。

## 客户面问题：统计口径只有一份

「未闭环 / 逾期 / 重要紧急」这几个数有**两个出口**——「客户面状态 · 问题跟踪」顶部的统计卡，
与度量看板的「客户面问题」看板（按战场 / 业务组 / 分类专项）。口径收口在
[routers/_customer_issue_stats.py](backend/routers/_customer_issue_stats.py) 的 `summarize()`，
两处共用；各写一份的表现是同一批条目在两个页面上给出不同的条数，而两边看着都对。

- **未闭环 ＝ `status != "CLOSED"`**。四档状态里 OPEN / 挂起 / 待升级版本都算，
  新增状态档天然落在这一边，不用回来改（见「枚举：单一来源」客户面状态那条）。
- **逾期同理只排除 CLOSED**，且「超过」才算、当天到期不算。
- **`overdue_unknown` 要一起报**：计划解决时间是选填的，整组都没填时「逾期 0」会被读成
  "一条都没超期"。看板上未闭环全都没填时直接摆一条黄条说明"算不出来"。
- **分组与合计走同一个 `summarize`**，所以各行相加恒等于合计。分组各写一份判断的话，
  分项加起来和总数差几条，而每一行单独看都对。回归里专门钉了这一条。
- **没填某个维度的行要有自己那一桶并排最后**（`unassigned`，如「未分类」「未指定责任领域」）。
  藏起来的话，那批最该被捞出来补录的行就永远没人去补（同领域质量表的「未指定领域」）。
  业务组一维**只认 `group_id` FK**，不回退到 `owner_group` 自由文本——回退会让同一个组按
  两种写法分成两行，加起来还对，没人会去核。
- **这一页不跟顶部的「度量项目」走**：客户面问题挂在机台上，没有 `roadmap_projects` 这个维度。
  页面上要写明白，否则选了上面那个却什么都没变，看着像页面坏了（同「问题单超期」那一页）。
- 汇总页的统计卡是**四张**（总数 / 已闭环 / 重要紧急 / 待升级版本），`.stat-row` 的
  `grid-template-columns` 必须跟着卡片数改——写死 5 列放了 6 张卡，第 6 张会单独掉到第二行。
  「逾期」降级成筛选栏里的「只看逾期」勾选框（带角标），**能力不能跟着卡片一起没**。
- 看板是**图 + 表**：图看构成、表看细节，两者同一份数据同一个维度。
  **堆叠的四段必须互斥**（OPEN / 挂起 / 待升级版本 / 已闭环，加起来正好是总数）——
  「重要紧急」「逾期」是横切的，与状态有重叠，堆进去会得到一根比总数还长的条，
  而每一段单独看都对。它们只进表格。纯 OPEN ＝ `open − on_hold − pending_upgrade`。
  图只画前 12 行（维度取值可能上百个，全画出来每根条细得看不见），表格仍是全量。
- **图和表都点得进去**：跳到「客户面状态 · 问题跟踪」并按这一维筛好。
  「未指定」那一桶传的是 `unassigned=<维度>` 而不是某个 id——那一维本来就没值，
  硬编一个 id 过去会筛出一张空表，看着像跳错了页。跳过去**一律带上"含已闭环"**：
  看板的条数是全量算的，问题跟踪页默认只看未闭环，不带的话两边条数对不上而都看着对。
- 回归见 [tests/test_customer_issue_board.py](backend/tests/test_customer_issue_board.py)。
...
- **客户面问题的状态是四档**（`CUSTOMER_ISSUE_STATUSES`：OPEN / CLOSED / 挂起 / 待升级版本），
  **和领域事务风险的三档不再是同一份词表**——「待升级版本」＝问题在某个版本里已经改好、
  等现场升级上去才算完，领域那边没有"等现场升级"这一步，同步过去只会多一个没人选的值。
  加一档不是加个字符串就完的，四件事得跟着定，这也正是它**不做成配置项**的原因：
  ① 算不算未闭环（算——全系统的未闭环判定一律是 `status != "CLOSED"`，新档天然落在这边，
  统计卡、隐藏已闭环、到期提醒都不用改）；② 算不算逾期（算，见 `_is_overdue`：
  版本没升上去，问题在客户那儿就还在。开特例的话「逾期未闭环」会少一截，而没人说得清
  少的是哪些）；③ 排第几（`CUSTOMER_ISSUE_STATUS_RANK`，后端 `_sort_key` /
  `pptx_utils.issues_to_text` 与前端 `STATUS_RANK` 同款——分叉的表现是同一批条目在页面和
  PPT 里顺序不一样，而两边看着都对）；④ 长什么样（行底色、格子里的标签、PPT 前缀）。
  **汇总里要单独数出来**（`pending_upgrade`）：混在 `open` 里就分不出"还没人动"和
  "改好了等升级"，而这两种要追的是完全不同的人。
  回归见 [tests/test_customer_issue_status.py](backend/tests/test_customer_issue_status.py)，
  其中一条专门盯着「每一档都要在排序表里有位置」——漏配不报错，只是那一档静默排到最后。
...
同一次保存可能三条都要走，但它们答的是三个问题，**不要相互替代或合并**：

| 模块 | 答的问题 | 存什么 |
| --- | --- | --- |
| [op_log.py](backend/op_log.py) `log_op()` | 谁在什么时候动过这一条（**审计**） | 语义摘要 `fields=...`，**不写正文、不写敏感字段** |
| [notify.py](backend/notify.py) `dispatch()` / `broadcast()` | 该让谁知道（**通知人**） | 显式收件人 + `Subscription` 订阅表 |
| [revisions.py](backend/revisions.py) `record()` | 改之前写的是什么（**回看**） | 改前 / 改后的**值本身** |

- 合成一张表的表现是：审计页被几 KB 富文本撑爆（本来是按人按时间扫的），
  而按字段翻历史又得去解析那段摘要。
- op_log 与 notify 的异常都被吞掉，绝不阻塞主业务；**留痕不吞**——见下一节。

## 回头看：修订历史（连续）与整页存档（离散）

「某个时间点这里写的是什么」有两条实现，**都要留着，别合并**：
修订历史精确到每次保存但只覆盖登记过的列；整页存档一周一份但一眼看到全貌，
且不受行增删影响。只有前者的话，重建一张表要把行的创建时间与删除记录一起算，
重建错了出来的还是一张看着挺正常的表，没人会去核。

**修订历史**（[revisions.py](backend/revisions.py) + `field_revisions` 表）：

- 哪些实体、哪些列留痕，**收口在 `TRACKED` 一张表**。各 router 只调
  `snapshot()`（改之前拍一份）+ `record()`（改完比对），不要在自己那边另写判断，
  否则同一次保存在两张表上留下的痕迹深浅不一，而两边看着都对。
- **必须在 `db.commit()` 之前调，且 `record()` 自己不 commit**：留痕搭调用方的车，
  与改动同一个事务。能脱离改动单独存在的痕迹会造出「历史里写着改过、数据却没改」
  ——那比没有更糟。这一条与 `log_op` 相反，`log_op` 晚一点写没关系。
- **只记改动，不记新增**：行自己的 `created_at` 已经说明了"那时它还不存在"。
  **删除要记**（`action="delete"`，`old_value` 是整行 JSON）：误删掉的是别人
  跟了几周的进展。
- **按行号翻历史必须以「这一行什么时候建的」为下界**（`born_at()`）。SQLite 的
  `INTEGER PRIMARY KEY` 会**复用**已删行的 id，不设下界的话新建的空行会把前一个
  同号行的历史整段认领过来——页面上是刚建的一条，历史里却写着别人三周前的进展。
- 列的中文名取**模型上的 `comment`**（`field_label()`），不另建对照表；
  只有注释写的是取值范围（`OPEN / CLOSED / 挂起`）的那几列在 `_LABELS` 里单独指定。
- **大块 JSON 不留痕**（自由表格 / 阵型 / 里程碑）：一次保存动辄几十 KB，
  逐次留痕会把库撑大好几倍，而它们的回看诉求本来就是"那一周整页长什么样"。
  `machine_cells_json` / `risks_json` 是例外——它们是一小串状态，且正是天天在改的那部分。
- 出接口额外给 `old_html` / `new_html`（过 `_rich_to_html()`）：留痕里存的是**当时库里
  那个值**，而入口清洗是后加的，老数据没洗过。**前端渲染这两个，不要 v-html 原始值。**

**整页存档**（[archives.py](backend/archives.py) + `page_snapshots` 表）：

- `payload_json` 存的是**当时那一页的数据**，不是画面。存 HTML 的话样式一改，
  老存档就永远停在旧版式上。
- **专项的回看直接复用周报那一份渲染**（`_render_report_html`，payload 还原成
  `SimpleNamespace` 鸭子类型对象喂进去）。另写一套的表现是「加了一种分段，
  页面和周报里都有、存档里没有」。渲染时**必须把存档日传进去**，
  否则页头写的是今天，一份三个月前的档看着像今天刚出的报告。
- 同一对象**同一天覆盖**（`create_snapshot`）：定时重跑、手工再点都不会堆一摞。
- **图片不进存档**，按文件名去 `uploads/` 现取并内联成 `data:`（浏览器不认 `cid:`）。
  代价是图被换掉之后存档里也跟着变——这一条要在页面上写明白。
- 每周一 07:00 由 `scheduler.weekly_page_archive` 跑。**单个对象失败只跳过它并打日志**：
  吞掉的表现是「某一页从某周起就没有存档了」，等有人回头找那一周才发现。
- 权限：读＝登录用户；**手工存一份＝登录用户**（大改之前先存一档的正是动手那个人，
  要 admin 代存就没人会存了）；**删存档＝仅 admin**（按"删掉的是什么"定档，
  这是别人回溯要用的历史）。**历史没有写接口，也不该有。**
...
- **`fill_*_fk` 判的是「FK 有没有值」，不是「key 在不在字典里」**（`_fk_already_set()`）。
  按 key 判的话**新建那条路整个失灵**：`create_item` 用的是全量 `model_dump()`
  （不是 `exclude_unset`），`*_id` 这些可选列一律以 `None` 出现在字典里，于是守卫恒为真、
  反查一次都不跑。表现是「新建的需求填了版本号却没有 FK，编辑过的有」——两条路各走各的，
  页面上都显示着版本号，看不出差别；直到「版本管理 → 合入需求」按 FK 查交付范围时，
  新建的那批整片不出现。回归见
  [tests/test_req_bulk.py](backend/tests/test_req_bulk.py) 的
  `test_create_resolves_version_fk_like_edit_does`。
...
- **新增页面**＝在 [router/index.js](frontend/src/router/index.js) 加一条，`meta` 填
  `title` / `icon` / `group`（决定侧栏 8 分组归属）/ 按需 `requireAdmin` / `hidden`。
  **分组名必须在 App.vue 的 `GROUP_ORDER` 里**（概览 / 计划管理 / 客户面管理 / 进度管理 /
  质量管理 / 组织管理 / 知识管理 / 系统管理）——写了个不在表里的分组名，那一页在侧栏里
  直接消失，而路由还在、直接敲地址也进得去，最难发现的那类。
  路由守卫只拦 `requireAdmin`，页面内的 admin 自查是额外一层，**不能替代服务端校验**。
  **组内的先后顺序＝路由在 `routes` 数组里的声明顺序**（`menuGroups` 只按
  `GROUP_ORDER` 分组，组内保持原序）：「计划管理」里的里程碑 → 版本 → WBS
  是由粗到细排的，挪动这三块的相对位置就会改掉侧栏顺序。
- **要导出的 SVG，画笔一律写成元素属性，不要写在 `<style scoped>` 里**
  （见 [VersionTimeline.vue](frontend/src/components/VersionTimeline.vue)）：
  scoped CSS 靠 data-v 属性挂在页面 DOM 上，`XMLSerializer` 序列化出去的那份带不走它，
  表现是「页面上好好的，导出来网格线和文字全变成黑的默认色」。
  也**不能把 `<style>` 塞进模板里的 `<svg>` 内部**——Vue 的模板编译器把
  `<style>` / `<script>` 当副作用标签整个丢掉（dev 下直接编译报错、整页起不来，
  而 `vite build` 只是警告），属性是唯一两边都成立的写法。
