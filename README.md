...
| 计划管理 | 版本管理 | `/versions` | 登录用户 | 按里程碑项目分 Tab，三层逐级展开（大版本 → 版本 → 迭代版本），版本与迭代版本都带**实际发布日期**（发布后不再出现在迭代管理的「计划交付版本」下拉里），大版本带主干/分支状态与「设为主干」；顶部**版本时间轴**可切节点粒度（只看大版本 / 版本 / 迭代 B 版）、时间范围可选「全部（自动收窄到有内容的区间）/ 近 N 月 / 按月份区间」，计划中未发布的节点标成琥珀色，节点少时整体放大一档，并可**导出 PNG / SVG**；另有「现场调试版本」Tab（T 版本 + 诉求收集 + 接收人清单） |
...
| 质量管理 | 度量看板 | `/metrics` | 登录用户 | 五个 Tab：版本质量（口径＝整个版本，跨迭代）/ 领域质量（口径＝一个迭代）/ 组级负载 / 问题单超期 / 客户面问题。前两个都按领域分行给出代码量、用例密度、问题单密度；版本质量还带采集问题单并显式标注匹配率。完成率按进展子项加权（已完成 1.0、进行中 0.5、不涉及不计入）。顶部「度量项目」下拉只作用于前三个 Tab（后两个不是项目维度）；**客户面问题**看板按战场 / 业务组 / 分类专项三个维度统计（口径与「问题跟踪」统计卡同一份，不跟「度量项目」走） |
