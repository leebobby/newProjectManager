...
| 客户面管理 | 客户面状态 | `/customer-status` | 登录用户 | 三个 Tab：**机台总览**（机台编号/客户/型号/阶段/现场版本/关注度/进展/关键特性点灯）、**问题跟踪**（全战场问题条目汇总，**默认只看未闭环**、分页 50 条/页，可按客户/责任领域/状态过滤，支持新增/导入导出；新增时先选客户再选机台——条目始终挂在机台上）、**硬件问题清零**（一行一问题，尾部按机台展开清零状态）。总览顶部「编辑」开关切换只读/编辑 |
