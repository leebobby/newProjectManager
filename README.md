...
| 计划管理 | 版本管理 | `/versions` | 登录用户 | 按里程碑项目分 Tab，三层逐级展开（大版本 → 版本 → 迭代版本），版本与迭代版本都带**实际发布日期**（发布后不再出现在迭代管理的「计划交付版本」下拉里），大版本带主干/分支状态与「设为主干」；顶部**版本时间轴**可切节点粒度（只看大版本 / 版本 / 迭代 B 版）并**导出 PNG / SVG**；另有「现场调试版本」Tab（T 版本 + 诉求收集 + 接收人清单） |
| 计划管理 | WBS | `/wbs` | 登录用户（删整份 admin） | 一份 WBS 挂在一个专项或一台机台的调试上，层数不限；叶子行填人天 / 完成度 / 状态 / 计划起止，父行由服务端按人天加权汇总。3/4 级计划 |
| 客户面管理 | 客户面状态 | `/customer-status` | 登录用户 | 三个 Tab：**机台总览**（机台编号/客户/型号/阶段/现场版本/关注度/进展/关键特性点灯）、**问题跟踪**（全战场问题条目汇总，**默认只看未闭环**、分页 50 条/页，可按客户/责任领域/状态过滤，状态四档 **OPEN / 挂起 / 待升级版本 / CLOSED**（「待升级版本」＝已在某版本改好、等现场升级，仍算未闭环与逾期，顶部有独立统计卡），支持新增/导入导出；新增时先选客户再选机台——条目始终挂在机台上）、**硬件问题清零**（一行一问题，尾部按机台展开清零状态）。总览顶部「编辑」开关切换只读/编辑 |
