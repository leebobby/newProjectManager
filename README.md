...
│       ├── business_trips.py        /business-trips 客户面支撑记录 + 支撑看板（人天口径在此）
...
| 计划管理 | 里程碑管理 | `/roadmaps` | 仅 admin | 甘特式路线图，可管理项目 / 阶段 / 里程碑。项目级的 1/2 级计划 |
| 计划管理 | 版本管理 | `/versions` | 登录用户 | 按里程碑项目分 Tab，三层逐级展开（大版本 → 版本 → 迭代版本），版本与迭代版本都带**实际发布日期**（发布后不再出现在迭代管理的「计划交付版本」下拉里），大版本带主干/分支状态与「设为主干」；顶部**版本时间轴**可切节点粒度（只看大版本 / 版本 / 迭代 B 版）、时间范围可选「全部（自动收窄到有内容的区间）/ 近 N 月 / 按月份区间」，计划中未发布的节点标成琥珀色，节点少时整体放大一档，并可**导出 PNG / SVG**；另有「现场调试版本」Tab（T 版本 + 诉求收集 + 接收人清单）；迭代版本行可看「合入需求」（该版本的**交付范围**，已变更的需求不计入并注明剔除条数） |
| 计划管理 | WBS | `/wbs` | 登录用户（删整份 admin） | 一份 WBS 挂在一个专项或一台机台的调试上，层数不限；叶子行填人天 / 完成度 / 状态 / 计划起止，父行由服务端按人天加权汇总。3/4 级计划 |
| 客户面管理 | 客户面状态 | `/customer-status` | 登录用户 | 三个 Tab：**机台总览**（机台编号/客户/型号/阶段/现场版本/关注度/进展/关键特性点灯）、**问题跟踪**（全战场问题条目汇总，**默认只看未闭环**、分页 50 条/页，顶部四张统计卡（总数 / 已闭环 / 重要紧急 / 待升级版本），可按客户/责任领域/状态/只看逾期过滤，状态四档 **OPEN / 挂起 / 待升级版本 / CLOSED**（「待升级版本」＝已在某版本改好、等现场升级，仍算未闭环与逾期，顶部有独立统计卡），支持新增/导入导出；新增时先选客户再选机台——条目始终挂在机台上）、**硬件问题清零**（一行一问题，尾部按机台展开清零状态）。总览顶部「编辑」开关切换只读/编辑 |
...
| 进度管理 | 迭代详情 | `/iterations/:id` | 登录用户 | 两个 Tab：**产品需求**（7 个进展子项）/ **领域需求**（6 个进展子项）；均支持行内编辑、Excel 批量导入、导出 PPT；可**多选后批量改计划交付版本**（先选大版本再选构建）或**批量挪到下个月的迭代**（批量也走乐观锁，改不动的逐条列出）。两张表之间可**挂拆解关联**（一条产品需求由哪几条领域需求承接，多对多、可跨迭代）：产品需求侧显示「N/M 条完成」与未拆解条数，领域需求侧显示它是为哪些产品需求做的 |
...
| 进度管理 | 专项总览 | `/specials/overview` | 登录用户 | 一张跨专项的横表：序号 / 关键专题 / 目标 / **风险（红黄绿点灯）** / 关键进展 / 关键风险和措施 / 责任人。**六列全部从各专项自己的字段自动提取**，总览上不另存一份；其中「关键进展」与「关键风险和措施」可以**就地编辑**——改的是专项详情页里的那一段本身，所以详情页、周报、导出跟着一起变；点灯按「风险和问题」分段自动判（未登记＝未评估灰、全闭环＝绿、有未闭环＝黄、有未闭环且已过计划闭环时间＝红），也可在本页人工指定（带 ✎ 标记，可改回自动）。可**导出 PPT**（与页面同口径，副标题里报灯分布与人工指定的项数）。侧栏在「专项管理 · 总览」 |
...
| 质量管理 | 度量看板 | `/metrics` | 登录用户 | 五个 Tab：版本质量（口径＝整个版本，跨迭代）/ 领域质量（口径＝一个迭代）/ 组级负载 / 问题单超期 / 客户面问题。前两个都按领域分行给出代码量、用例密度、问题单密度；版本质量还带采集问题单并显式标注匹配率。完成率按进展子项加权（已完成 1.0、进行中 0.5、不涉及不计入）。顶部「度量项目」下拉只作用于前三个 Tab（后两个不是项目维度）；**客户面问题**看板按战场 / 业务组 / 分类专项三个维度做**图 + 表**（堆叠条形图看状态构成，图和表都可点击跳到「问题跟踪」并按该维筛好），口径与「问题跟踪」统计卡同一份，不跟「度量项目」走 |
| 组织管理 | 干系人管理 | `/stakeholders` | 登录用户（写 admin） | 项目组沟通地图 + 战场沟通矩阵 + **关键特性责任人**（按「关键特性 × 项目」给出 FO / SE / TFO，FO/SE 留空时继承「关键特性」页上的特性级责任人并标出来） + **项目阵型**（阵型图上传 / 工时名单，支持 Excel 导入导出） |
...
- **问题单 API 采集**：新增按项目（`config.issue_api_projects`）的每日快照体系——
  ...
  采集侧三道过滤中**只有状态剔除与部门过滤会丢行**；责任人归不到小组的行保留为
  「未归组」并列成待办（`GET /api/issues/ungrouped`）——「新增/解决」是相邻快照的
  编号差集、不读状态，丢一行就等于凭空多一笔「解决」。同理采集脚本翻页时逐页校验
  且最后比对 `data.total`，拉不全就整次失败、不落短快照。
...
- **度量看板**：版本质量 / 领域质量 / 组级负载三个视图（调试版本看板在版本管理页）；完成率按进展子项
  加权（已完成 1.0、进行中 0.5、未开始与已延期 0、不涉及不计入分母）。
- **PPT 导出**：三处（客户面状态 / 迭代需求 / 缺陷统计报表）共用
  [pptx_utils.py](backend/pptx_utils.py) 的排版引擎——按估算行高自动分页、
  列宽归一化铺满页宽、长文本按行截断、宽矩阵按列切页、合计行加粗、URL 列转可点链接。
- **领域管理增强**：事务与风险跟踪 `domain_risks`（跨领域，带责任领域 + 责任人 +
  优先级 + 风险等级四列，当前进展为富文本）、不管理的 PL 组软删除 `domain_hidden`。
  **优先级与风险等级是两个口径**：前者答"先处理哪个"、后者答"爆了有多疼"，
  合成一列就表达不出"低优先级的高风险"（短期不动、但爆了很惨）；风险等级**可留空**，
  因为这张表里事务行和风险行混着，事务本来就没有等级。
- **客户面支撑情况**：`business_trips` 支撑记录（人 / 战场 / 支撑项目 / 现场·线上 /
  起止 / 事由 / 工作量人天），状态按日期实时推导不入库；看板按区间统计人次与人天，
  可按项目与支撑方式收窄。工作量口径：`man_days` 填了以它为准、留空按日历天数推导，
  跨区间按重叠天数比例分摊（见 CLAUDE.md「客户面支撑：工作量（人天）口径」）。
...
- **问题单每日新增 / 解决**（新表 `issue_snapshot_flows`，`create_all` 自动建）：快照只存
  ...
  快照里，明细回上一次快照的文件取）。
- **专项分段新增「里程碑」形态 + 单元格富格式**（Alembic `0009`）：
  自定义分段从三种（表格 / 文本框 / 图片）扩到四种，新增**里程碑**——与内置「计划」
  同一条时间轴，但节点存在分段自己身上，一个专项可以有多条子计划各自成段；
  周报文本、周报 HTML、Excel 三处与内置计划共用渲染函数。
  表格单元格与文本框都可逐格 / 逐段设**字体、字号、加粗、斜体、下划线、字色、底色**，
  富文本另加对齐与有序/无序列表；字体存 key 不存 CSS 串，页面 / 周报 / Excel 三处
  各自映射（词表见 `enums.GRID_FONTS` 与 [utils/gridFormat.js](frontend/src/utils/gridFormat.js)）。
  「解决方案专项」模板补三段：**解决方案验收标准 / 总体测试策略 / 专项测试计划**，
  排在「测试详细进展和点灯」之前；模板种子只在空表时注入，故存量库靠 `0009` 按
  `tkey` 指纹认领并补段（只增不删，已改过的列名与已填数据不动）。
- **富文本入口清洗**：目标 / 整体进展 / 求助 / 事务 / 风险 / 文本框分段的 HTML 改为
  **写库前**过白名单（此前只在导出周报时清洗，页面自身的 `v-html` 无防护）；
  `script` / `style` 连内容一起丢弃，白名单同步放开 `ul`/`ol`/`li` 与
  `text-align` / `background-color`，否则新加的列表与高亮按钮每次保存被静默抹掉。
- **修复 Excel 空分段占用章节号**：导出侧原先「先写标题、发现空了再补一行—」，
  与页面/周报的「空段整段跳过」不一致；模板里多一个默认空分段，Excel 的章节号就与
  周报错位。改为标题挂起、第一笔内容落笔时才写。
- **领域管理 · 遗留问题**：新增 Tab 与表 `domain_legacy_issues`——编号 / 任务名称 /
  状态（**OPEN / CLOSED / pending**）/ 当前责任人 / 提出人 / 确认人（三个人员下拉）/
  参与人（多选，存用户 id JSON）/ 所属领域 / 计划完成时间 / 优先级 /
  **当前进展（富文本，支持加粗与颜色）**。协作编辑域，带乐观锁。
  当前进展在页面上用 `v-html` 渲染，所以**写库前**就过 `_sanitize_rich()` 白名单——
  只在导出时清洗等于只保护了收件人，页面本身仍是任何登录用户都能往别人的条目里存脚本。状态入口归一大小写（`enums.norm_domain_legacy_status`），否则
  "Pending" 与 "pending" 在统计里会分成两档。
- **领域管理 · 问题单情况改接问题单管理**：从读本地 Excel 报表改为读
  **`issue_snapshots` 的最新一次快照**，顶部按项目（YLS3000 / YLS5000 …）切换；
  只看最新一份，历史趋势仍在问题单管理里看。指定的项目还没采集过时，如实显示
  「未接入」并给出原因，**不回退成别的项目的数字**；一份快照都没有的部署回退到
  原 Excel 口径，不丢这一列。
- **领域管理 · 问题单目标**：新增 `domain_issue_targets`（领域 × 项目，数量目标 +
...
- **客户面支撑看板增强**（Alembic `0011`）：`business_trips` 加 `project_id`（支撑项目，
  挂 `roadmap_projects`）/ `support_mode`（现场支撑 · 线上支撑）/ `man_days`（工作量人天）
  三列。看板顶部可按项目与支撑方式收窄（同时作用于 now 快照与区间统计），
  区间统计除人次外直接给出**工作量人天**（合计 + 现场/线上拆分），
  维度卡扩到 领域 / 人 / 战场 / 项目 / 支撑方式 五张并按人天排序。
- **人天口径**：`man_days` 填了以它为准，留空按起止日历天数推导（含头含尾）；
  区间统计按**重叠天数占整段的比例分摊**，跨月的支撑不会在每个月各算一遍。
  线上支撑常是「五天各两小时」，建议手填，否则会被算成 5 人天。
  老数据由 `0011` 全部回填成「现场支撑」，项目与人天留空。
- **问题跟踪页提速**：两个日期列改成「点开才挂 `el-date-picker`」的文本单元格，
  行内下拉加 `:persistent="false"`，表格分页 50 条/页。原因是 Element Plus 的
  date-picker 面板立即渲染且无法关掉，几百行就是几万个 DOM 节点。
- **问题跟踪默认只看未闭环**：「含已闭环」复选框默认不勾（导出跟随该开关）；
  点顶部「已闭环」统计卡会自动放开该开关，不会得到一张空表。
- **版式模板并入专项配置**：原顶层菜单「专项模板」下线，改为「专项管理 · 专项配置」页的
  第二个 Tab；`/special-templates` 保留为重定向，老书签不会 404。
- **版本三层自愈**（Alembic `0012`）：`0010` 若在某个库上跑到一半（列已加、部分版本已生成），
  重跑会被它自己的「结果表非空就跳过」守卫挡住，剩下的迭代版本停在
  `release_version_id IS NULL`，在页面上不可见——现象是「迁移完只剩第一层」。
  `0012` 把这些孤儿行按同一套 `B\d+` 规则挂回去，**只挂接不删除**，可反复执行。
  另有 [scripts/repair_version_tiers.py](backend/scripts/repair_version_tiers.py)：
  默认只读体检 + 修复预演，`--apply` 才写库，并能顺手清掉「本身就是版本」的冗余行。
- 版本管理页大版本行新增「下辖 N 版本 / M 构建」两个计数，收起态就能看出下面是不是空的。
- **版本顺序自己排 + 放错位置能挪**：三层的操作列各加「上移 / 下移」，顺序按
  `sort_order` 持久化，整体提交（`POST /{tier}/reorder`，只带 `{parent_id, ids}`）——
  逐个 PUT `sort_order` 中途失败会留下「排到一半」的顺序，而顺序错了不报错。
  同时去掉了这三张表的列排序（含迭代表原来写死的 `default-sort`，它每次都按版本号
  重排，手排的顺序根本落不到眼里）。
  版本 / 迭代版本的编辑弹窗新增「所属大版本」「所属版本」下拉，改了就是整个挪过去：
  版本挪走时名下构建跟着走，挪过去的行**排在新父级末尾**（带着旧序号会插进中间，
  看着像随机落点）。
- **迭代需求分项目 + 度量看板按项目度量**（Alembic `0013`）：领域需求与产品需求各加一列
  「项目」（`project_id` → `roadmap_projects`），可在表格里直接点选，也支持按项目筛选
  （含「未指定项目」一档，用来把待补录的老数据捞出来）；两份 Excel 导入模板各多一列「项目」，
  按项目名完全匹配，对不上留空不报错。
  项目挂在**需求行**上而不是迭代上——迭代是按年月排的，同一个月里排着多个项目的需求。
  度量看板顶部新增「度量项目」下拉，作用于版本完成率 / 迭代质量 / 组级负载三个 Tab。
  **按项目筛时，没填项目的行不计入任何一个项目**（摊进去数字看着都合理，偏了没人看得出来），
  改为由接口回一个 `unassigned`，页面用一条黄条如实说明「还有 N 条需求没填项目，未计入本次统计」。
  老数据的 `project_id` 一律留空，需要人工补录。
- **「已变更」的需求整行不进统计**：任一进展子项标了「已变更」＝这条需求本轮不做了。
  迭代管理里该行**整行置灰**并在标题前挂一个「已变更」标签，工具条上另有「隐藏已变更」开关
  （默认不勾——默认隐藏的话，误标的行会连同「改回来」的入口一起消失）。
  度量看板四个口径与领域总览的需求统计一律不计入它，并各回一个 `changed` 如实报出
  被排除的条数（只剔不报的表现是「升级完数字小了一截」，没人说得清为什么）。
  判定收口在 [enums.py](backend/enums.py) 的 `is_changed_row()`，前后端同款。
  ⚠️ 这是对旧口径的**反转**：原来「已变更」按 0.5 加权算作"变更后仍在做"，
  会让一条已经不做的需求持续把平均完成度往下拽。无迁移，升级后数字即刻变化。
- **客户面问题跟踪支持直接新增**：汇总页工具条加「新增」按钮。此前条目只能在
  「客户面状态」总览的单元格里建（那里机台是现成的），汇总页跨战场、没有机台上下文，
  于是弹窗里先选客户再选机台——条目在库里始终挂在机台上（`customer_id` 由机台推导，
  前端传了也不算数）。问题 / 需求 / 事务三类都能建，与该页显示的范围一致。
  正按某个客户/领域筛着时，新建的条目默认落在该筛选里，否则建完立刻被自己的筛选藏起来；
  真被藏起来时提示语会明说「已新增（当前筛选条件下不显示）」。
- **迭代需求按项目分标签**：领域需求 / 产品需求两张表上方各加一排项目标签
  （全部 / 各项目 / 未指定项目，标签上带条数），替换掉原来的「按项目筛选」下拉。
  两个 Tab 共用同一个项目选择（由 `IterationDetail` 持有），来回切时筛选跟着走；
  停在某个项目标签上时新增的需求默认归它。切标签不重新请求，只是前端切片。
- **PPT 导出重排版**：三处导出（客户面状态 / 迭代需求 / 缺陷统计报表）以前把所有数据行
  画在**一张** slide 上，行一多表格就顺着页面往下长、长到幻灯片外面去——导出来一半内容
  看不见，得手工拆页。现在统一走 [pptx_utils.py](backend/pptx_utils.py) 的
  `add_table_slides()`：按**估算行高**自动分页（不是按固定条数，一行可能是 6 行文字也
  可能是 1 行）、页码与「第 n/N 页」写在页眉页脚、每页重复表头；列宽改成传**比例**并
  归一化到正好铺满页宽（原来四张表里三张的英寸数都对不上，不是越界就是右边空一条）；
  长文本列按行截断并写明「另 N 条」（一台机器挂十几条事务时，那一行原本能顶满整张页）；
  宽矩阵（客户/月份很多）**按列切页**并在每页带上标签列（原来 23 个客户挤一页，
  每列只剩 0.4 英寸）；合计行加粗加底色；问题单 URL 列渲染成可点的「查看」。
  顺带修了产品需求表里「交付进展跟踪」父表头**多罩了三列**（预估/实际代码量、实际工作量）
  的问题。版面回归见 [tests/test_pptx_layout.py](backend/tests/test_pptx_layout.py)。
- **度量看板改按「版本 / 领域」两个质量维度重排**：从四个 Tab 收成三个——
  「版本质量」（口径＝整个版本，跨迭代）、「领域质量」（口径＝一个迭代，按月排活）、
  「组级负载」。两个质量 Tab 都按领域分行给出需求数 / 完成度 / 代码量 / 自验证用例 /
  用例密度 / 转测后问题单 / 问题单密度，行形状与前端表格组件都是同一份
  （`metrics.DomainQualityRow` + [DomainQualityTable.vue](frontend/src/components/metrics/DomainQualityTable.vue)），
  避免两个维度长出两套列名。质量字段只有领域需求有，所以版本口径的「总需求」含产品需求、
  按领域表只数领域需求，两个数对不上是正常的，表头写明了。
  版本质量还多两列**采集问题单**：按快照行的「版本信息」精确匹配到本版本
  （版本号 + 名下所有构建号，与 `_req_scope` 同一份口径），并在表上方**显式标注命中率**
  与没命中的取值——DTS 那边是自由串，对不上是常态，不报命中率的话「这个版本怎么
  一个问题单都没有」没人说得清。问题单数据源的选取收口到新的
  [routers/_issue_source.py](backend/routers/_issue_source.py)，领域总览与度量看板共用。
  明细表与年度趋势收进折叠面板；**「调试版本」看板移到「版本管理 → 现场调试版本」**
  （它按客户×月统计，和其它三个不是一个维度）。
- **领域总览支持按版本统计**：页头新增一排口径标签「按迭代 / 按版本」（只有这两档），
  具体哪个月、哪个版本由旁边的下拉选。按版本时统计的是该**版本**（C10SPC101 这一层）
  上的需求，**跨迭代**，此时忽略月份选择（两者叠加会得到一个既不是这个版本、
  也不是这个迭代的数）。版本下拉只列当前挂着需求的版本并带条数；版本的匹配规则（名下所有构建 + 字符串回退）
  与度量看板的版本达成率共用一份实现（[routers/_req_scope.py](backend/routers/_req_scope.py)），
  否则同一个版本在两个页面上条数会不一样，而两边看着都像对的。
