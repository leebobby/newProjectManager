...
from routers._req_progress import PRODUCT_PROGRESS_FIELDS, split_changed
...
@router.get("/by-version", response_model=schemas.IterationProductRequirementScopeOut)
def list_by_version(
    version_id: int = Query(..., description="迭代版本 ID（按 target_version_id 过滤）"),
    db: Session = Depends(get_db),
):
    """版本管理用：这个迭代版本的**交付范围**（计划交付版本指向它的产品需求）。
    ...
    **「已变更」的行不进交付范围**：那条需求本轮不做了，留在版本的合入清单里会让
    ...
    kept, changed = split_changed(items, PRODUCT_PROGRESS_FIELDS)
    pmap = project_name_map(db)
...
def list_duplicates(
    ...
    所以摆出来让人自己判断，而不是替人决定。
    """
    return scan_duplicates(db, models.IterationProductRequirement, iteration_id)
