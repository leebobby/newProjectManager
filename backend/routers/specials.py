...
# 填在详情页里的东西应该自己长到这儿来。
#
# 不变量是**总览不存自己的一份**，而不是"总览上不许改"：页面上「关键进展」与
# 「关键风险和措施」是可以就地编辑的，但走的是 `PUT /{sid}/content` 与
# `PUT /risks/{item_id}`——改的是详情页那个字段本身，所以两边永远是同一个值。
# 真正要拦的是「在总览上单独存一列」：那一列一旦存在，它就会和详情页对不上，
# 而两边看着都对。**新增列时照这个办：要么从已有字段推，要么写回已有字段，
# 不要给总览开自己的存储。**
